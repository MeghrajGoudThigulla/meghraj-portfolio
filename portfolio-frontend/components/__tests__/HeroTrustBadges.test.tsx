...
describe("HeroTrustBadges telemetry", () => {
  ...
  it("tracks trust badge impressions once per session", async () => {
    const badges = HERO_TRUST_BADGES.map((badge) => ({ ...badge, id: `${badge.id}-impression` }));
    render(<HeroTrustBadges badges={badges} />);
    await waitFor(() => {
      const events = trackMetricMock.mock.calls.map(([payload]) => payload as { eventName?: string }).filter((payload) => payload.eventName === "hero_trust_badge_impression");
      expect(events).toHaveLength(badges.length);
    });
    cleanup(); render(<HeroTrustBadges badges={badges} />);
    await waitFor(() => {
      const events = trackMetricMock.mock.calls.map(([payload]) => payload as { eventName?: string }).filter((payload) => payload.eventName === "hero_trust_badge_impression");
      expect(events).toHaveLength(badges.length);
    });
  ...
  it("tracks badge click events", async () => {
    ...
    const badges = HERO_TRUST_BADGES.map((badge) => ({ ...badge, id: `${badge.id}-click` }));
    render(<HeroTrustBadges badges={badges} />);
